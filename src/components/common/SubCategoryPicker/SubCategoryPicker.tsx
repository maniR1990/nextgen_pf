...
export function SubCategoryPicker({ categories, value, onChange, label }: SubCategoryPickerProps) {
  ...
  const filteredChildren = activeParent?.children.filter(ch =>
    !search.trim() || ch.label.toLowerCase().includes(search.toLowerCase())
  ) ?? [];
    ...
    <div className="subcat-picker">
      ...
      <div className="subcat-picker__panes">
        ...
        <div className="subcat-picker__left">
          <div className="subcat-picker__pane-header" aria-hidden>Category</div>
              ...
              >
                <div className="subcat-picker__parent-left">
                  ...
                  <div className="subcat-picker__parent-text">
                    ...
                    {selectedChild && (
                      <span className="subcat-picker__parent-selected">{selectedChild.label}</span>
                    )}
                  </div>
                </div>
        ...
        <div className="subcat-picker__right">
          {activeParent && (
            <div className="subcat-picker__pane-header" aria-hidden>{activeParent.label}</div>
          )}
          ...
          <div role="listbox" aria-label={activeParent?.label ?? 'Subcategories'}>
            ...
            {filteredChildren.map(child => (
              ...
              >
                ...
                {child.id === value && <Check size={14} className="subcat-picker__check" aria-hidden />}
              </button>
            ))}
          </div>
        </div>
        ...
        {mobileView === 'child' && activeParent && (
          <>
            <button
              ...
              onClick={() => { setMobileView('parent'); setSearch(''); }}
            >
            ...
            <div className="subcat-picker__mobile-list" role="listbox" aria-label={activeParent.label}>
              {filteredChildren.length === 0 && (
                <div className="subcat-picker__empty">No results</div>
              )}
              {filteredChildren.map(child => (
