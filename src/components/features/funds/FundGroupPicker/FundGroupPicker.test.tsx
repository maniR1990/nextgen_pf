...
describe('FundGroupPicker', () => {
  it('renders fund group dropdown', () => {
    render(<FundGroupPicker groups={mockGroups} value={null} flow={null} onChange={vi.fn()} />);
  ...
  it('shows IN/OUT direction radios when a group is selected', () => {
    render(<FundGroupPicker groups={mockGroups} value="fg1" flow="IN" onChange={vi.fn()} />);
  ...
  it('OUT radio is checked when flow=OUT', () => {
    render(<FundGroupPicker groups={mockGroups} value="fg1" flow="OUT" onChange={vi.fn()} />);
  ...
  it('calls onChange with fundGroupId and flow=IN when group first selected', async () => {
    ...
    render(<FundGroupPicker groups={mockGroups} value={null} flow={null} onChange={onChange} />);
  ...
  it('calls onChange with flow=OUT when OUT radio clicked', async () => {
    const user = userEvent.setup();
    const onChange = vi.fn();
    render(<FundGroupPicker groups={mockGroups} value="fg1" flow="IN" onChange={onChange} />);
  ...
  it('shows empty-state message when no groups exist', () => {
    render(<FundGroupPicker groups={[]} value={null} flow={null} onChange={vi.fn()} />);
