...
export function PointsRedeemForm({ values, errors, onChange }: Omit<PointsRedeemFormProps, 'paymentSources'> & { paymentSources?: PaymentSourceOption[] }) {
  const ptsNum = parseFloat(values.ptsSpent);
  const rateNum = parseFloat(values.ptsRate);
  const autoAmount =
  ...
  return (
    <div className="tx-form tx-form--points">
      ...
      <div className="tx-form__row">
        <FormField label="Merchant / Programme" htmlFor="tx-merchant" error={errors.merchant} required>
          <input
            id="tx-merchant"
            type="text"
        ...
        <FormField label="Points Spent" htmlFor="tx-pts-spent" error={errors.ptsSpent} required>
          <input
            ...
            step="1"
            min="1"
            className={['form-input', errors.ptsSpent && 'form-input--error'].filter(Boolean).join(' ')}
        ...
        >
          <input
            ...
            step="0.01"
            min="0.01"
            className={['form-input', errors.ptsRate && 'form-input--error'].filter(Boolean).join(' ')}
      ...
      {autoAmount && (
        <div className="tx-form__auto-amount">
          ...
          <span className="tx-form__auto-value">₹{parseFloat(autoAmount).toLocaleString('en-IN')}</span>
        </div>
      )}
