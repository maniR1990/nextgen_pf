...
export const v1CreateFund = compose(withAuth(), withValidation(CreateFundSchema))(async (req, ctx) => {
  try {
...
export const v1UpdateFund = compose(withAuth(), withValidation(UpdateFundSchema))(async (req, ctx) => {
  try {
    const id = ctx.params?.id;
    if (!id) return missingId();
...
export const v1AllocateFund = compose(withAuth(), withValidation(AllocateFundSchema))(
  async (req, ctx) => {
    try {
      const id = ctx.params?.id;
      if (!id) return missingId();
...
export const v1SaveFundAllocations = compose(withAuth(), withValidation(SaveAllocationsSchema))(
  async (req, ctx) => {
    try {
      const id = ctx.params?.id;
      if (!id) return missingId();
...
export const v1RemoveFundAllocation = compose(withAuth())(async (_req, ctx) => {
  try {
    ...
    if (!id || !accountId) {
      return v1FromApiError({ message: 'Missing id or accountId', status: 400, code: 'VALIDATION_ERROR' });
    }
