...
const handleReportsKpi = compose(withAuth())(async (req, ctx) => {
  ...
  try {
    const [incomeAgg, expenseAgg, investAgg, atmAgg, incomeGroups, sipCount, budgetSummary] =
      await Promise.all([
        prisma.financeTransaction.aggregate({
          where: { userId, budgetPeriodYear: year, budgetPeriodMonth: month, type: { in: INCOME_TYPES as never } },
          _sum: { amount: true },
        }),
        prisma.financeTransaction.aggregate({
          where: { userId, budgetPeriodYear: year, budgetPeriodMonth: month, type: { in: EXPENSE_TYPES as never } },
          _sum: { amount: true },
        }),
        prisma.financeTransaction.aggregate({
          where: { userId, budgetPeriodYear: year, budgetPeriodMonth: month, type: { in: INVEST_TYPES as never } },
          _sum: { amount: true },
        }),
        prisma.financeTransaction.aggregate({
          where: { userId, budgetPeriodYear: year, budgetPeriodMonth: month, type: { in: ATM_TYPES as never } },
          _sum: { amount: true },
        }),
    ...
    const incomeSourceLabel = incomeGroups
    ...
    const expensesPct =
      expensesBudget > 0
        ? Math.round((expensesSpent / expensesBudget) * 1000) / 10
        : 0;
    ...
    // SIP count label
    const investedLabel =
      sipCount > 0 ? `${sipCount} SIP${sipCount !== 1 ? 's' : ''}` : '—';
